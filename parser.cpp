...
namespace Parser {
	...
	std::string getNextToken(std::string program_cmd, int& start_pos) {
		...
		if (program_cmd[token_start_i] == '<' || program_cmd[token_start_i] == '>') {
			start_pos = program_cmd.find_first_not_of(" ", token_start_i + 1);
			return program_cmd.substr(token_start_i, 1);
		} else {
			int token_end_i = program_cmd.find_first_of(">< ", token_start_i);
			token_end_i = ((token_end_i == std::string::npos) ? program_cmd.length() - 1 : token_end_i - 1);
			start_pos = program_cmd.find_first_not_of(" ", token_end_i + 1);
	...
	program_cmd_t getParsedProgramCmd(const std::string program_cmd) {
		...
		program_exec_i_end = ((program_exec_i_end == std::string::npos)
			? program_cmd.length() - 1 : program_exec_i_end - 1);

		program_cmd_t parsed_program_cmd;
		std::string program_exec = program_cmd.substr(program_exec_i_start, program_exec_i_end - program_exec_i_start + 1);
		parsed_program_cmd.program_exec = program_exec;
		parsed_program_cmd.args.push_back(program_exec);

		int current_i = program_exec_i_end + 1;
		while (current_i < program_cmd.length()) {
